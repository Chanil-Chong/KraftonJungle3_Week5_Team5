...
void FViewport::EnsureResources(ID3D11Device* Device)
{
	...
	if (FAILED(Device->CreateDepthStencilView(DepthStencilTexture, &DepthDSVDesc, &DepthStencilView)))
	{
		Release();
		return;
	}
	...
	if (FAILED(Device->CreateShaderResourceView(DepthStencilTexture, &DepthSRVDesc, &DepthStencilSRV)))
	{
		Release();
		return;
	}
...
{
	IUnknown* Resource = reinterpret_cast<IUnknown*>(DepthStencilSRV);
